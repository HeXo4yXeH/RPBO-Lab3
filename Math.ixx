...
export int GreatestCommonDivisor(int a, int b)
{
	int r;
	if (a < 0)
		a *= -1;
...
export int LeastCommonMultiple(int x, int y) {
...
public:
	void normalize()
	{
		int nod = GreatestCommonDivisor(m_nominator, m_denominator);
		m_nominator /= nod;
		m_denominator /= nod;
		if (m_denominator < 0) {
			m_denominator *= -1;
	...
	Rational& operator+=(Rational _obj) {
		int new_den = LeastCommonMultiple(m_denominator, _obj.m_denominator);
	...
	Rational& operator-=(Rational _obj) {
		int new_d = GreatestCommonDivisor(m_denominator, _obj.m_denominator);
...
export Rational operator+ (const Rational& _obj1, const Rational& _obj2) {
	int denominator = LeastCommonMultiple(_obj1.m_denominator, _obj2.m_denominator);
...
export bool operator>(const Rational& _obj1, const Rational& _obj2)
{
	int den = LeastCommonMultiple(_obj1.m_denominator, _obj2.m_denominator);
