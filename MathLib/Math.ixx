...
public:
	...
	Complex(double real) {//конструктор преобразования
		m_re = real;
		m_im = 0.0;
	}
	Complex(double real, double image) {//конструктор инициализации
		m_re = real; 
		m_im = image;
	}
	...
	static Complex FromAlgebraicForm(double real, double image) {
...
export Complex f(const Complex& z) {
	int a_real = 0, a_imag = 1;
	Complex a(a_real, a_imag);
	Complex result = 2 * z + sin(double(z - a));
	return result;
}

export Rational f(const Rational& r) {
	int a_nom = 1, a_den = 2;
	Rational a = a_nom / a_den;
	Rational result = 2 * r + sin(double(r - a));
	return result;
}

export double f(double x) {
	int a = 0.5;
	double result = 2 * x + sin(x - a);
	return result;
}